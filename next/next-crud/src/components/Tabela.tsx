...
function CabecalhoTabela(props: TabelaProps) {
...
function DadosTabela(props: TabelaProps) {
    const exibirAcoes = props.clienteExcluido || props.clienteSelecionado
            ...
            {props.clientes.map((cliente, i) => {
                return (
                    <tr key={cliente.id} className={`${i % 2 === 0 ? 'bg-purple-200' : 'bg-purple-100'}`}>
                        ...
                        {exibirAcoes ? (BotoesAcoes(cliente, props)) : false}
                    </tr>
                )
            })}
        </tbody>
...
function BotoesAcoes(cliente: Cliente, props: TabelaProps) {
    return (
